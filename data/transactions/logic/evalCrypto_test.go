...
func benchmarkBn256DataGenData(b *testing.B) (data []benchmarkBn256Data) {
	...
	for i := 0; i < b.N; i++ {
		...
		data[i].g2, _ = hex.DecodeString("")
...
func BenchmarkBn256AddRaw(b *testing.B) {
	data := benchmarkBn256DataGenData(b)
	a1 := new(bn254.G1Affine)
	a1.Unmarshal(data[0].g1)
	a2 := new(bn254.G1Affine)
	a2.Unmarshal(data[0].g1)
...
func BenchmarkBn256PairingRaw(b *testing.B) {
	...
	for i := 0; i < len(g1Bytes)/64; i++ {
		g1 := new(bn254.G1Affine)
		g1.Unmarshal(g1Bytes[i*64 : i*64+64])
		g1s = append(g1s, *g1)
	}

	var g2s []bn254.G2Affine
	for i := 0; i < len(g2Bytes)/128; i++ {
		g2 := new(bn254.G2Affine)
		g2.Unmarshal(g2Bytes[i*128 : i*128+128])
		g2s = append(g2s, *g2)
	}
